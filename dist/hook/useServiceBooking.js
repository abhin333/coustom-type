...
export function useServiceBooking(data) {
    ...
    const getServiceOptions = useMemo(() => (category) => {
        const categoryServices = CategoryServices[category]?.services || [];
        return ((data?.products?.results || []).sort((a, b) => {
    ...
    const getSizeOptions = useMemo(() => (serviceId) => {
        const service = data?.products?.results.find((product) => product.id == serviceId);
    ...
    const getServicePrice = useMemo(() => (serviceTypeId, sizeId) => {
        const product = data?.products?.results.find((product) => product.id == serviceTypeId);
        const priceInPaise = product?.options.find((option) => option.id == sizeId)?.price;
